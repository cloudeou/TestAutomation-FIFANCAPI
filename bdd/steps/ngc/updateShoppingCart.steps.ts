...
export const updateShoppingCartSteps = ({
    ...
    when('user try to update Shopping Cart', async () => {
        console.log("user try to update Shopping Cart")
        ...
        console.log("prevResponse",responseText);
        console.log("lpdsid",externalLocationId);
        ...
        console.log("ecid",customerAccountECID);
        console.log("offersToAdd",selectedOffers);
    ...
    then(/^validate shopping cart is updated successfully$/, async () => {
        ...
        bodyParser.validateOfferMapInResponse(childOfferMap, response);
        bodyParser.validateOfferMapNotInResponse(childOfferMapDeleted, response);

        bodyParser.validateCharMapInResponse(
            childOfferMap,
            shoppingCartContext().getCharMap(),
            response,
            shoppingCartContext().getExistingChildOffers(),
        );
        if (!bodyParser.validateWorkOrdersCorrectness(response)) {
            console.warn(
    ...
    then(/^validate that offers can not be removed$/, async () => {
        ...
        bodyParser.validateAllOffersPresentInResponse(response, offersDeleted);
        return;
    });
    ...
    and(
        ...
        (priceType) => {
            ...
            SCResponseBody = responseContext().getSCresponse();
